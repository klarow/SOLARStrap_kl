...
class SolarException(Exception):
    pass
#KL9 add empi2cov here
def build_solar_directories(h2_path, iid2ped, empi2trait,
fam2empi, fam2count, fam2proband, use_proband, trait_type, empi2cov, cov_list,
verbose = True, family_ids_only = None):
    ...
    for row in trait_ped:
        ...
        if trait is None:
            trait = ''
            age = ''
        #KL9 updated to take into account different # of covariates
        if empi2cov == None:
            if use_proband:
...
#KL10 updated with covariates
    ...
    #KL11 make cov_list a list if None for tcl
    if cov_list == None:
        cov_list = [""]
...
#KL 6 add in empicov and covlist
...
use_proband, house=False, nprocs=1, verbose=False, buildonly=False, empi2cov=None, cov_list=None):
...
#KL7 pass cov stuff to solar
def solar(h2_path, families_with_case, icd9, trait_type, num_families,
iid2ped, all_traits, eth, fam2empi, fam2eth,all_fam2count, all_fam2proband,
use_proband, house, verbose, buildonly, empi2cov, cov_list):
    """
...
#KL8 pass cov stuff to build solar strap
